...
{
    int frame, seed, iterations, nFrames;
    double roughness;
    DrawState *ds;
    Module *scene;
    Module *terrain;
    ...
    vector_set(&(view.vpn), 0.0, -3.0, 5.0);
    ...
    for (frame = 0; frame < nFrames; frame++)
    {
        ...
        module_module(scene, terrain);

        module_identity(scene);
        module_scale(scene, .02, .02, .02);
        module_rotateY(scene, 0, 1);
        module_translate(scene, -1.0 + 3.0 * ((double)frame / (double)nFrames), 0.75, 1.0 - 3.0 * ((double)frame / (double)nFrames));
        module_module(scene, xwing);

        matrix_rotateY(&GTM, cos(M_PI / 60.0), sin(M_PI / 60.0));
        module_draw(scene, &VTM, &GTM, ds, light, src);

        sprintf(buffer, "terrainFlying-frame%03d.ppm", frame);
