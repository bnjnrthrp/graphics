...
int main(int argc, char *argv[])
{
	...
	// Generate the random numbers between 0 and 1
	for (int i = 0; i < 16; i++)
	{
		random[i] = (double)rand() / (double)RAND_MAX;

		point_set3D(&p[i], cos(2.0 * M_PI * i / 16.0), sin(2.0 * M_PI * i / 16.0), (double)rand() / (double)RAND_MAX);
	}
	...
	for (frame = 0; frame < 60; frame++)
	{
		...
		sprintf(buffer, "creativeSurface-3-frame%03d.ppm", frame);
