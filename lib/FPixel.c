...
void convert_Pixel(Pixel *pixel, int rows, int cols, FPixel *dest)
{
    ...
    {
        pixel[i].r = uc_to_float(dest[i].rgb[0]);
        pixel[i].g = uc_to_float(dest[i].rgb[1]);
        pixel[i].b = uc_to_float(dest[i].rgb[2]);
    }
}
...
void convert_FPixel(FPixel *fpixel, int rows, int cols, Pixel *dest)
{
    for (int i = 0; i < rows * cols; i++)
    {
        dest[i].r = float_to_uc(fpixel[i].rgb[0]);
        dest[i].g = float_to_uc(fpixel[i].rgb[1]);
        dest[i].b = float_to_uc(fpixel[i].rgb[2]);
    }
};
